-- JustCocon booking core schema (DRAFT — review only; do not apply without approval)
-- Target: staging project fnisoydbemhhfnmgydmc
-- Scope: additive public tables only; does NOT reference or alter calls / call_intelligence
--
-- Relationship model (acyclic MVP):
--   customers 1—* bookings
--   bookings 1—0..1 conversation_sessions.booking_id (session points to booking after confirm)
--   conversation_sessions 1—* whatsapp_messages
...
-- ---------------------------------------------------------------------------
-- bookings (no FK to conversation_sessions — avoid circular dependency)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- conversation_sessions (booking_id → bookings; single direction association)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- audit_logs (recommended for MVP observability; append-only usage)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Row Level Security (staging MVP: service-role backend only)
-- ---------------------------------------------------------------------------
...
-- Intentionally no CREATE POLICY for anon/authenticated: default deny for non-bypass roles.
-- Backend whatsapp-service uses SUPABASE_SERVICE_ROLE_KEY (bypasses RLS).
